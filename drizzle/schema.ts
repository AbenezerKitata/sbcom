import { pgTable, uniqueIndex, foreignKey, pgEnum, text, timestamp, boolean, integer, serial, varchar, doublePrecision } from "drizzle-orm/pg-core"
  import { sql } from "drizzle-orm"

export const vehicleStatus = pgEnum("vehicle_status", ['UNAVAILABLE', 'AVAILABLE'])
...
export const post = pgTable("Post", {
	...
	updatedById: text("updatedById").references(() => user.id, { onDelete: "set null", onUpdate: "cascade" } ),
	deletedById: text("deletedById").references(() => user.id, { onDelete: "set null", onUpdate: "cascade" } ),
},
(table) => {
	return {
...
export const resShuttle = pgTable("resShuttle", {
	...
	shuttleId: text("shuttleId").notNull().references(() => shuttleAssign.id, { onUpdate: "cascade" } ),
});
...
export const newVeh = pgTable("new_veh", {
	...
	amount: integer("amount").notNull(),
	attachedId: text("attachedID").notNull().references(() => attached.id, { onDelete: "cascade", onUpdate: "cascade" } ),
});
...
export const unattachedData = pgTable("UnattachedData", {
	id: text("id").primaryKey().notNull(),
	groupsId: text("groupsId").notNull().references(() => groups.id, { onDelete: "cascade", onUpdate: "cascade" } ),
...
export const license = pgTable("License", {
	...
	name: text("name"),
	description: text("description"),
	locationId: text("locationId").notNull().references(() => location.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	waiverId: text("waiverId").references(() => smartWaiver.waiverId, { onDelete: "set null", onUpdate: "cascade" } ),
});
...
export const bookings = pgTable("bookings", {
	id: text("id").primaryKey().notNull(),
	whenBooked: timestamp("when_booked", { precision: 3, mode: 'string' }).notNull(),
...
export const vehicle = pgTable("vehicle", {
	id: text("id").primaryKey().notNull(),
	vehicleTypeId: text("vehicleTypeId").notNull().references(() => vehicleType.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	...
	registrationExpiry: timestamp("registration_expiry", { precision: 3, mode: 'string' }),
	cfn: text("cfn"),
...
export const additionalfeesOffer = pgTable("additionalfees_offer", {
	id: text("id").primaryKey().notNull(),
	offerId: text("offer_id").notNull().references(() => offer.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	additionalfeesId: text("additionalfees_id").notNull().references(() => additionalfees.id, { onDelete: "restrict", onUpdate: "cascade" } ),
});

export const discountOffer = pgTable("discount_offer", {
	id: text("id").primaryKey().notNull(),
	offerId: text("offer_id").notNull().references(() => offer.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	discountId: text("discount_id").notNull().references(() => discountrate.id, { onDelete: "restrict", onUpdate: "cascade" } ),
});

export const peakRateOffer = pgTable("peakRate_offer", {
	id: text("id").primaryKey().notNull(),
	offerId: text("offer_id").notNull().references(() => offer.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	peakId: text("peak_id").notNull().references(() => peakrate.id, { onDelete: "restrict", onUpdate: "cascade" } ),
});
...
export const vehicleOffer = pgTable("vehicle_offer", {
	id: text("id").primaryKey().notNull(),
	offerId: text("offer_id").notNull().references(() => offer.id, { onDelete: "restrict", onUpdate: "cascade" } ),
	vehicleId: text("vehicle_id").notNull().references(() => vehicle.id, { onDelete: "restrict", onUpdate: "cascade" } ),
